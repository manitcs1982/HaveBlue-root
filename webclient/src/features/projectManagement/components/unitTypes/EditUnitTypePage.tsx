...
import { useUpdateUnitType } from "../../projectManagementMutations";

import { useAuthContext } from "../../../common/AuthContext";
import { ErrorMessage } from "../../../common/ErrorMessage";
import { processErrorOnMutation } from "../../../../util/errorMessaging";

export const EditUnitTypePage = () => {
  ...
  const { mutateAsync: mutate } = useUpdateUnitType();
  const {
    error: errorUnitTypeDetails,
    data: unitTypeDetails,
  ...
  if (
    ...
    return (
      <>
        ...
        {isErrorUnitTypeDetails && (
          <ErrorMessage error={errorUnitTypeDetails} />
        )}
      </>
    );
  }
      ...
      <Container>
        ...
        >
          <Grid item xs={12}>
            <Formik
              initialValues={{
                manufacturer: unitTypeDetails.manufacturer,
                description: unitTypeDetails.description,
                notes: unitTypeDetails.notes,
                model: unitTypeDetails.model,
                number_of_cells:
                  unitTypeDetails.module_property.number_of_cells,
                nameplate_pmax: unitTypeDetails.module_property.nameplate_pmax,
                module_width: unitTypeDetails.module_property.module_width,
                module_height: unitTypeDetails.module_property.module_height,
                system_voltage: unitTypeDetails.module_property.system_voltage,
                auditor: unitTypeDetails.module_property.auditor,
                audit_date: unitTypeDetails.module_property.audit_date,
                audit_report_id:
                  unitTypeDetails.module_property.audit_report_id,
                isc: unitTypeDetails.module_property.isc,
                voc: unitTypeDetails.module_property.voc,
                imp: unitTypeDetails.module_property.imp,
                vmp: unitTypeDetails.module_property.vmp,
                alpha_isc: unitTypeDetails.module_property.alpha_isc,
                beta_voc: unitTypeDetails.module_property.beta_voc,
                gamma_pmp: unitTypeDetails.module_property.gamma_pmp,
                cells_in_series:
                  unitTypeDetails.module_property.cells_in_series,
                cells_in_parallel:
                  unitTypeDetails.module_property.cells_in_parallel,
                cell_area: unitTypeDetails.module_property.cell_area,
                unit_type_family: unitTypeDetails.unit_type_family,
                module_technology:
                  unitTypeDetails.module_property.module_technology,
                bifacial: unitTypeDetails.module_property.bifacial,
              }}
              validationSchema={Yup.object({
                ...
                auditor: Yup.string().nullable(),
                audit_date: Yup.string().nullable(),
                audit_report_id: Yup.string().nullable(),
              ...
              onSubmit={async (values, { setSubmitting }) => {
                try {
                  ...
                  if (values.audit_date instanceof Object) {
                    tempDate = values.audit_date.format("YYYY-MM-DD");
                  } else {
                    tempDate = values.audit_date;
                  }
                  ...
                  toast.success("Unit Type was succesfully updated");
                } catch (error) {
                  toast.error("Error while updating unit type.");
