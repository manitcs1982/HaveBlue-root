class InsertNapaTransit():
    ...
    MAGIC
    To use this plugin:
    POST {"serials":["serial_1","serial_2", ... "serial_n"]}
    """
    def __init__(self:object):
        self.tsd = 99 #whatever it is

    def _test(self:object):
        return(None)

    def _run(self:object, serials:list=None):
        from lsdb.models import TestSequenceDefinition
        from lsdb.models import Unit
        from django.core.exceptions import ObjectDoesNotExist

        try:
            new_tsd = TestSequenceDefinition.objects.get(id=self.tsd)
        ...
        for serial in serials:
            ...
            try:
                ...
                response_dict[serial] = "something went wrong building buckets"
    ...
    def _build_bucket(self:object, work_order:object, test_sequence:object, leg:float, unit:object):
        ...
        for execution in test_sequence.procedureexecutionorder_set.all():
            ...
            if not execution.execution_condition == None:
                retval:bool = False
                exec('retval={}'.format(execution.execution_condition))
