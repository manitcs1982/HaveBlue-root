...
import { DIODE_TEST_UNITS as diodeTestUnits } from "../../util/constants";
...
export const DiodeTestModal = ({
  ...
  const { getTableProps, headerGroups, rows, prepareRow } = useTable({
    columns,
    data: isSuccessView
      ? getDataFromMeasurementResults(viewData, diodeTestUnits)
      : [],
    initialState: {
      hiddenColumns: ["id"],
    },
  ...
  const handleRetestAction = async () => {
    try {
      ...
      toast.success("Diode Test was sent for retesting");
      ...
      toast.error("Error while retesting diode test");
      setConfirmationDialogRetestOpen(false);
      setOpen(false);
  ...
  const handlePassAction = async () => {
    try {
      ...
      toast.success("Diode Test Review Approved");
      setConfirmationDialogApproveOpen(false);
      setOpen(false);
      queryClient.invalidateQueries("verify_data");
      queryClient.invalidateQueries("travelerResults");
    } catch (error) {
      toast.error("Error while approving diode test");
      setConfirmationDialogApproveOpen(false);
      setOpen(false);
      processErrorOnMutation(error, dispatch, history);
    }
  ...
  return (
    <div>
      ...
      >
        ...
        <ConfirmationDialog
          id="diode-test-retest"
          keepMounted
          open={confirmationDialogRetestOpen}
          title="Diode Test Retest Confirmation"
        ...
        />
        <ConfirmationDialog
          id="diode-test-approve"
          keepMounted
          open={confirmationDialogApproveOpen}
          title="Diode Test Approval Confirmation"
