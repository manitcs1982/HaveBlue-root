...
import { ActionDetailModal } from "./actionCreation/ActionDetailsModal";

export const WorkOrderActionsTable = ({ reports = [] }: any) => {
        ...
        Cell: ({ cell, row } : { cell: Cell; row: any }) => {
          return (
            <Button
              variant="contained"
              color="primary"
              onClick={() => {setAction(row.original.id); setOpen(!open);}}
            >
              {cell.value}
            </Button>
          )
  ...
  return (
    ...
    <Grid container direction="row" justify="space-between" alignItems="center">
      ...
      <ActionDetailModal id={action} open={open} setOpen={setOpen} workorder={{name : "TestData"}} />
    </div>
